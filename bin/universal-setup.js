...
`);
      ...
      // Create Docker documentation for Python projects
      if (!fs.existsSync('DOCKER.md')) {
        fs.writeFileSync('DOCKER.md', `# Docker Setup for ${config.projectName}

## 📦 Available Dockerfiles

Your project includes multiple Docker configurations:

### 🐍 \`Dockerfile\` (Recommended for Production)
- **Purpose**: Optimized Python application container
- **Base**: \`python:3.11-slim\`
- **Size**: ~200-400MB
- **Includes**: Only Python runtime and your requirements
- **Use for**: Production deployments, CI/CD, containerized apps

\`\`\`bash
# Build and run your Python application
docker build -t ${config.projectName.toLowerCase()} .
docker run -p 8000:8000 ${config.projectName.toLowerCase()}
\`\`\`

### 🛠️ \`Dockerfile.universal\` (For Development Only)
- **Purpose**: Full development environment with tools
- **Base**: \`node:20-bullseye-slim\`
- **Size**: ~2-3GB
- **Includes**: Python, Node.js, CLI tools, browsers for testing
- **Use for**: Development containers, devcontainer setup

\`\`\`bash
# Build development environment (takes longer)
docker build -f Dockerfile.universal -t ${config.projectName.toLowerCase()}-dev .
docker run -p 8000:8000 -v $(pwd):/workspace ${config.projectName.toLowerCase()}-dev
\`\`\`

## ⚡ Quick Start

For most Python projects, use the main \`Dockerfile\`:

1. **Make sure you have a \`requirements.txt\`** (already created)
...
- Use **"Reopen in Container"** - it will automatically use the universal setup
- Your code will be mounted at \`/workspace\`
- All development tools are pre-installed

## 🎯 Which Dockerfile Should I Use?

| Scenario | Use | Build Time | Size |
|----------|-----|------------|------|
| **Production deploy** | \`Dockerfile\` | ~2-5 min | Small |
| **CI/CD pipeline** | \`Dockerfile\` | ~2-5 min | Small |
| **Local development** | \`Dockerfile\` | ~2-5 min | Small |
| **VS Code devcontainer** | \`Dockerfile.universal\` | ~10-15 min | Large |
| **Need browser testing** | \`Dockerfile.universal\` | ~10-15 min | Large |
...
spec:
  ...
  selector:
    matchLabels:
      app: ${config.projectName.toLowerCase()}
  template:
    metadata:
      labels:
        app: ${config.projectName.toLowerCase()}
    spec:
      containers:
      - name: app
        image: ${config.projectName.toLowerCase()}:latest
        ports:
        - containerPort: 8000
\`\`\`

---
*Generated by Universal Dev Environment*
`);
      }
